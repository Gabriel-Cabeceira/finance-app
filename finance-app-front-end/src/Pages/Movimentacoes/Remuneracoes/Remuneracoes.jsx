...
import { Divisor, LoadContainer, Main, Resume } from "./Remuneracoes.styles";
...
export default function Remuneracoes() {
    ...
    const fetchData = async (date) => {
        try {
            const response = await fetch(`${apiUrl}/api/incomes?date=${date}`);
    ...
    const handleSubmit = async (formData) => {
        try {
            const response = await fetch(`${apiUrl}/api/incomes`, {
                method: 'POST',
                headers: {
    ...
    return (
        <Container>
            <Divisor>
                <AddButton title="Adicionar Remuneração" onClick={handleOpenModal} />
                <MonthCarousel
